...
public class EmulatorTest extends EmulatorTestAbstract {
  ...
  public void testCsiSaveRestoreCursor() throws IOException {
    TestSession session = new TestSession(30, 3);
    session.process("start\r\n\u001B[sProgress 1%\u001B[u\u001B[KProgress 2%");

    assertScreenLines(session, List.of("start", "Progress 2%"));
  }
  ...
  public void testDeviceStatusReportWithOriginMode() throws IOException {
    ...
    session.process(csi("2;23r"));
    session.process(csi("?6h"));
    session.process(csi("1;1H"));

    Assert.assertEquals(2, session.getTerminal().getCursorY());

    session.process(csi("6n"));

    Assert.assertEquals("\u001B[1;1R", session.getTerminal().getOutputAndClear());
  }

  public void testNoScrollWhenCursorIsBelowScrollRegion() throws IOException {
    TestSession session = new TestSession(80, 5);
    session.process(csi("1;3r"));
    session.process("Line 1\r\nLine 2\r\nLine 3");
    session.process(csi("4;1H"));
