...
public class JediEmulator extends DataStreamIteratingEmulator {
  ...
  private boolean processControlSequence(ControlSequence args) {
    switch (args.getFinalChar()) {
      ...
      case 's':
        return saveCursor(args); // SCP (Save Cursor Position)
      case 't':
        return windowManipulation(args);
      case 'u':
        return restoreCursor(args); // RCP (Restore Cursor Position)
      default:
        return false;
    }
  }

  private boolean saveCursor(ControlSequence args) {
    if (isPrefixedOrParameterized(args)) {
      return false;
    }
  ...
  private boolean restoreCursor(ControlSequence args) {
    if (isPrefixedOrParameterized(args)) {
      return false;
    }
  ...
  private boolean isPrefixedOrParameterized(ControlSequence args) {
    return args.getCount() > 0 ||
           args.startsWithExclamationMark() ||
           args.startsWithQuestionMark() ||
           args.startsWithMoreMark() ||
           args.startsWithLessMark() ||
           args.startsWithEqualsMark();
  }
